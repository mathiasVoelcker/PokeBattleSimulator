class AttacksController < ApplicationController
  ...
  before_action :set_attack_types, only: [:index, :new, :show, :edit, :update, :destroy]
  # GET /attacks
  # GET /attacks.json
  def index
    @attacks = Attack.all
  ...
  # GET /attacks/1.json
  def show
    @element = @elements.find(@attack.element_id).name
  end
  ...
  private
    ...
    def set_attack_types
      @attack_types = AttackType.all
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def attack_params
      params.require(:attack).permit(:name, :base_power, :element_id, :attack_type_id)
    end
end
